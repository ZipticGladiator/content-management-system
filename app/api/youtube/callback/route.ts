...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(new URL(`/youtube?youtube_error=${encodeURIComponent(error)}`, request.url));
  }
  if (!code) {
  ...
  return NextResponse.redirect(new URL("/youtube?youtube_connected=1", request.url));
}
